# Equestrian Data Exchange Data Object Specification
...
[2.1. Standards and Formats](#2.1.-standards-and-formats)

[2.2 Links](#2.2-links)

[2.3 Dates and DateTimes](#2.3-dates-and-datetimes)

[2.4 Data Internationalization and Localization](#2.4-data-internationalization-and-localization)

[2.5 Schema Standards](#2.5-schema-standards)

[2.6 Entity Conventions](#2.6-entity-conventions)

[2.7 Geo-Location](#2.7-geo-location)

[3.1 Definition of “Required”](#2.8-definition-of-“required”)
...
[3.17 Geolocation Object](#3.17-geolocation-object)
...
This specification describes the data entities required to represent equestrian information and provides a structured data exchange model to support interoperability across various applications. The Equestrian Data Exchange Specification (EDES) is designed to be comprehensive yet flexible, supporting both basic data information and more complex structures.
...
* **Result**: The outcome of a competition.
...
## 2.8 Definition of “Required” {#2.8-definition-of-“required”}

Required means that the field MUST be present in the entity and can’t have an empty value or represent an empty composite object.
...
| CurrentName | string | Max length: 255 | The actual name of the horse that is used by its owners. |
...
| HorseId | array | Array of objects of type [“Identifier” 3.15](#3.15-identifier) Required | Foreign key referencing the Horse entity. |
| EquestrianId | array | Array of objects of type [“Identifier” 3.15](#3.15-identifier) Required | Foreign key referencing the Equestrian entity (typically the rider). |
...
| ScoreSchema | string | A valid URI to a Schema describes the additional fields of the specific type of a score object. Required | This is used to allow clients to validate and parse custom score objects. |
...
* **Fields**:  
  * **Value**: String, the ID’s actual value. The format and requirements vary based on the Type.  
  * **Type**: Enum, specifying the format of the ID, with values such as:  
    * **String**: Alphanumeric or plain string format.  
    * **UUID**: Universally unique identifier.  
    * **Integer:** Integer type representation of the value.  
  * **Entity**: Reference to the entity that owns this ID (e.g., Horse, Breeding Association).
...
| Type | enum | The type of the id |  |
| Entity | string | The type of the id |  |
...
* **Fields**:  
  * **ColorId**: Unique identifier for the color.  
  * **PrimaryColor**: Enum specifying a base color (e.g., Black, Bay, Chestnut, Gray).  
  * **ColorPattern**: Enum, specifying color patterns (e.g., Solid, Roan, Tobiano, Sabino).  
  * **ColorList**: Array of strings, allowing multiple colors or shades to be associated with a horse if a color is non-standard or has special characteristics (e.g., “Blue Roan”, “Palomino with Flecks”).

| Field Name | Type | Constraints | Description |
| :---- | :---- | :---- | :---- |
| ColorId | id | required |  |
| PrimaryColor | enum | required |  |
| ColorPattern | enum |  |  |
| ColorList | array |  |  |
...
* **Fields**:  
  * **CompanyId**: Composite ID as specified in section 3.9.  
  * **Name**: String, name of the company.  
  * **Type**: Enum, type of company (e.g., Sponsor, BreedingAssociation, Veterinary).  
  * **AddressId**: Foreign key, references the Address entity (see section 3.13).  
  * **ContactId**: Foreign key, references the Contact entity (see section 3.14).  
  * **Website**: URI, link to the company’s website.  
  * **RegistrationNumber**: String, unique identifier for the company, such as a commercial or tax registration number.  
  * **CountryOfRegistration**: ISO 3166-1 alpha-3 code, country where the company is registered.
...
| Field Name | Type | Constraints | Description |
| :---- | :---- | :---- | :---- |
| EventOrganizerId | object | Object of type [“Identifier” 3.15](#3.15-identifier) Required |  |
| Name | string | Required | Name of the event organizer |
| EventOrganizerType | string | Required | Enum (e.g., Individual, Company) |
| CompanyId | object | Object of type [“Identifier” 3.15](#3.15-identifier) | References a [company entity](#3.11-company) |
| AddressId | object | Object of type [“Identifier” 3.15](#3.15-identifier) | References an [address entity](#3.13-address) |
| ContactId | object | Object of type [“Identifier” 3.15](#3.15-identifier) | References a [contact entity](#3.14-contact) |

## 3.13 Address {#3.13-address}

The **Address** entity provides a standardized way to store postal addresses associated with individuals or organizations and other entities.
...
* **Fields**:  
  ...
  * **Country**: ISO 3166-1 alpha-3 code, country of the address.  
  * **Latitude**: Decimal, optional, latitude for geolocation, following [ISO 6709](https://en.wikipedia.org/wiki/ISO_6709).  
  * **Longitude**: Decimal, optional, longitude for geolocation, following [ISO 6709](https://en.wikipedia.org/wiki/ISO_6709).
...
| streetAddress | string | Max length 192 |  |
| streetAddress2 | string | Max length 192 |  |
...
| geolocation | object | Gelocation object as described in [3.17 Geolocation Object](#3.17-geolocation-object) | Longitude and latitude. |

## 3.14 Contact {#3.14-contact}

The **Contact** entity represents a contact point for individuals or organizations and other entities, including phone, email, and other contact methods.

* **Fields**:  
  * **ContactId**: Unique identifier for the contact.  
  * **PhoneNumber**: String, primary phone number.  
  * **Email**: String, primary email address.  
  * **AlternatePhone**: String, secondary phone number (optional).  
  * **FaxNumber**: String, fax number (optional).  
  * **PreferredContactMethod**: Enum, preferred method of contact (e.g., Phone, Email, Fax).  
  * **Website**: URI, optional, contact’s website link if applicable.
...
| PhoneNumberType | enum, PhoneNumberType | If none is provided “OTHER” is assumed. | The type of the phone number. |
...
* [https://en.wikipedia.org/wiki/ISO\_6709](https://en.wikipedia.org/wiki/ISO_6709)
...
* [Tim Donandt](mailto:t.donandt@clipmyhorse.tv) ~~Clarify what the industry wants for the horse genders, we assume that it is only gelding, mare and stallion.~~  
* [Tim Donandt](mailto:t.donandt@clipmyhorse.tv) clarify the use of association specific country codes, for example the IOC codes.  
  * [https://data.fei.org/NFPages/NF/Search](https://data.fei.org/NFPages/NF/Search)  
  * The FEI Provides a List of National Federations, we should use this list.   
    * Unclear how we can get information about updates

* [Tim Donandt](mailto:t.donandt@clipmyhorse.tv) clarify the use of colors. The current idea is to use one list defined as part of this standard. Additionally two separate fields COULD be used to select a subset of colors and the specific color from that subset which could be provided by 3rd parties e.g. breeding associations.


* [Tim Donandt](mailto:t.donandt@clipmyhorse.tv) figure out if we have very specific types of persons e.g. an athlete might require a valid riding pass, should we therefore have a dedicated “equestrian” or “rider” entity vs a more generic entity?  
  * Since individuals can take on various roles, I believe only persons exist. If a specific role requires particular information (e.g., a valid riding license), we should attach that information to the individual.  
* ✅ ~~[Florian Krämer](mailto:f.kraemer@clipmyhorse.tv) Add “Company” and “Event Organizer” and “Address”, “Contact” entities.~~  
* ~~[Florian Krämer](mailto:f.kraemer@clipmyhorse.tv)~~